...
void loop() {
  ...
  if (chidanIndex < chidan.length()) {
    ...
    // Dừng lại ở giao lộ kế tiếp sau khi hết chỉ dẫn
  ...
  return;
    ...
    if (sensorValues[0] == 0 && sensorValues[1] == 1) {
      moveMotors(L175, R255);  // Sharp right
    } else if (sensorValues[0] == 0 && sensorValues[1] == 0) {
      moveMotors(L195, R255);
      ...
      moveMotors(L255, R195);
    } else if (sensorValues[4] == 0 && sensorValues[3] == 1) {
      moveMotors(L255, R175);  // Sharp left
    } else {
